...
from tourelle_ros.srv import SetBool,SetBoolRequest,SetBoolResponse
#import pcl_ros
...
class pcl2_creator:
    def __init__(self):
        ...
        self.pub_acq = rospy.Publisher("/tourelle/pcl_acq",Bool,queue_size=3)
        ...
        print "Creation du service clear"
        self.service_clear = rospy.Service("/tourelle/pcl_creator_2d_node/clear",Empty,self.cb_service_clear)    #déclaration du service  
        print "Creation du service acq_enable"
        self.service_acq_enable = rospy.Service("/tourelle/pcl_creator_2d_node/acq_enable",SetBool,self.cb_acq_enable)    #déclaration du service
    ...
    def cb_acq_enable(self,req):
        self.acq_seq = req.data
    ...
    def process_pcl(self,force=False):
        if (self.process_cpl_flag or force):
            ...
            try:
                self.pcl.header.stamp = rospy.Time.now()
                self.listener_camera.waitForTransform('camera_link', 'base_link', rospy.Time.now(), rospy.Duration(0.1))
                #base_point.header.stamp = rospy.Time.now()
                print "PointCloud size",len(self.pcl.points)
                print "Go transformPointCloud"
                #start_time = timeit.default_timer()
        ...
        self.pub_acq.publish(self.acq_seq) #publi si True s'il y a acquisition sinon False                
...
if __name__=="__main__":
    ...
    print "Starting pcl_creator_2d node"
    rospy.init_node("pcl_creator_2d_node")
    ...
    while not rospy.is_shutdown():
        ...
        r.sleep()
    rospy.spin()
    rospy.loginfo("pcl_creator_2d Node terminated")
